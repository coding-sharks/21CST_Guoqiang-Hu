...
def load_stats_file(stats_file):
    ...
    with open(stats_file, 'r', encoding='utf8') as fin:
        for line in fin:
            ...
            threshold, fa_per_hour, far, frr = arr
            values.append([float(fa_per_hour), float(far) * 100, float(frr) * 100])
    values.reverse()
...
def plot_det_curve(
    ...
    for index, keyword in enumerate(keywords):
        ...
        plt.plot(values[:, 1], values[:, 2], label=keyword) # 0, 1
...
def plot_det_fa_hour_curve(
    ...
    for index, keyword in enumerate(keywords):
        stats_file = os.path.join(stats_dir, 'stats.' + str(index) + '.txt')
        values = load_stats_file(stats_file)
        plt.plot(values[:, 0], values[:, 2], label=keyword)
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        ...
        type=str,
        default='FAR',
        help='x-axis: FAR or FA_per_hour')
    ...
    if x_axis == 'FAR':
        plot_det_curve(
            ...
            args.ylim,
            args.y_step)
    elif x_axis == 'FA_per_hour':
        plot_det_fa_hour_curve(
            keywords,
            args.stats_dir,
            args.figure_file,
            args.xlim,
        ...
        raise ValueError('x_axia must be FAR or FA_per_hour')
