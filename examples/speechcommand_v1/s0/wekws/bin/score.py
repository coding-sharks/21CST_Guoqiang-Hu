...
def main():
    ...
    with torch.no_grad(), open(score_abs_path, 'w', encoding='utf8') as fout:
        for batch_idx, batch in enumerate(test_data_loader):
            ...
            num_keywords = logits.shape[1]  # (D) the number of keywords : ce loss
            logits = logits.cpu()

            if len(logits.shape) == 3:
                # streaming decode [B, T, Keywords]
                ...
                # global decode [B, Keywords + 1]
                logits = torch.nn.functional.softmax(logits, dim=-1)
                num_keywords = logits.shape[1] - 1
                for i in range(len(keys)):
                    key = keys[i]
                    score = logits[i]
                    for keyword_i in range(1, num_keywords + 1):
                        # keyword starts from 1
                        keyword_score = score[keyword_i]
                        fout.write('{} {} {:.6f}\n'.format(key, keyword_i, keyword_score))
            else:
                raise ValueError('Unsupported logits shape: {}'.format(logits.shape))
            if batch_idx % 10 == 0:
