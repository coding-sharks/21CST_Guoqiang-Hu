...
if __name__ == '__main__':
    ...
    min_far_plus_frr = float('inf')
    equal_error_rate = 0.0
    min_far_plus_frr_threshold = 0.0
    equal_error_rate_threshold = 0.0
    with open(args.stats_file, 'w', encoding='utf8') as fout:
        keyword_index = int(args.keyword)
        # threshold = 0.0
        # while threshold <= 1.0:
        for threshold in tqdm(np.arange(0.0, 1.0, args.step)):
            num_false_reject = 0
            ...
            if false_alarm_rate + false_reject_rate <= min_far_plus_frr:
                min_far_plus_frr = false_alarm_rate + false_reject_rate
                min_far_plus_frr_threshold = threshold

            if not equal_error_rate and false_alarm_rate <= false_reject_rate:
                equal_error_rate = false_alarm_rate
                equal_error_rate_threshold = threshold

    print(f"Equal Error Rate: {equal_error_rate:.6f}, Threshold: {equal_error_rate_threshold:.6f}")
    print(f"Min FAR + FRR: {min_far_plus_frr:.6f}, Threshold: {min_far_plus_frr_threshold:.6f}")
    result_file = os.path.dirname(args.stats_file) + '/result.txt'
    with open(result_file, 'w', encoding='utf8') as f:
        f.write(f"Equal Error Rate: {equal_error_rate:.6f}, Threshold: {equal_error_rate_threshold:.6f}\n")
        f.write(f"Min FAR + FRR: {min_far_plus_frr:.6f}, Threshold: {min_far_plus_frr_threshold:.6f}\n")
